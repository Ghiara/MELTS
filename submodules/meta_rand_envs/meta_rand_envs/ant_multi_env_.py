...
class AntMultiEnv(RandomEnv, utils.EzPickle):
    ...
    def sample_tasks(self, num_tasks_list):
        ...
        # # flipping
        # if 'flip' in self.task_variants:
        #     goals = np.linspace(self.velocity_y[0], self.velocity_y[1], num=sum(num_tasks_per_subtask))
        #     tasks_flipping = [{'base_task': self.bt2t['flip'], 'specification': goal, 'color': np.array([0.5, 0.5, 0])} for goal in goals]
        ...
        #
        # # stand_up
        # if 'stand' in self.task_variants:
        #     goals = np.linspace(self.pos_y[0], self.pos_y[1], num=sum(num_tasks_per_subtask))
        #     tasks_stand_up = [{'base_task': self.bt2t['stand'], 'specification': goal, 'color': np.array([1., 0, 0.5])} for goal in goals]
