...
class AntNonStationaryDirectionEnv(NonStationaryGoalDirectionEnv, MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        forward_reward = (xposafter - xposbefore) / self.dt * self.active_task
        ctrl_cost = 0.5 * np.square(action).sum()
        contact_cost = 0.5 * 1e-3 * np.sum(
            np.square(np.clip(self.sim.data.cfrc_ext, -1, 1)))
        survive_reward = 1.0
        reward = 10 * forward_reward - 1 * ctrl_cost - 1 * contact_cost + 1 * survive_reward
        if self.termination_possible:
            state = self.state_vector()
            notdone = np.isfinite(state).all() and state[2] >= 0.30 and state[
                2] <= 10.0  # original gym values: 0.2 and 1.0
            done = not notdone
        else:
            done = False
        self.steps += 1
        ...
        return ob, reward, done, dict(
            ...
            direction=(xposafter - xposbefore)
        )
