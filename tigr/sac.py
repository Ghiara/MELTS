...
class PolicyTrainer:
    ...
    def training_step(self, indices, augmented_buffer=False):
        ...
        if not augmented_buffer:
            ...
            if self.context_type == 'sample':
                # Option 1: sample from the posterior belief
                z, gammas = self.encoder(encoder_input, return_probabilities=True)
                task_z = z.detach().clone()
            else:
                # Option 2: parametrization of the posterior belief
                mu, log_var = self.encoder.encode(encoder_input)
                task_z = torch.cat((mu.detach().clone(), log_var.detach().clone()), dim=1)
        else:
