...
class AugmentedTrainer(BaseTrainer):
    def __init__(self, *args, **kwargs):
        ...
        self.optimizer_mixture_model = self.optimizer_class(
            [{'params': self.encoder.shared_encoder.parameters()},
             {'params': self.encoder.fc_alpha.parameters()},
             {'params': self.encoder.fc_beta.parameters()}],
            lr=self.lr_encoder
        )
    ...
    def mixture_training_step(self, indices):
        ...
        assert not torch.isnan(alpha).any(), alpha
        assert not torch.isnan(beta).any(), beta

        # Sample latent variables
        latent_variables = self.encoder.sample(latent_distributions, alpha=alpha, beta=beta)
        ...
        # KL divergence for Stick-breaking VAE
        prior_alpha = torch.Tensor([1.]).to(alpha.device)
        prior_beta = torch.Tensor([5.]).to(alpha.device)

        psi_b_taylor_approx = beta.log() - 1. / beta.mul(2) - 1. / beta.pow(2).mul(12)
        kld = ((alpha - prior_alpha) / alpha) * (-0.57721 - psi_b_taylor_approx - 1 / beta)
        kld += alpha.mul(beta).log() + self.beta_func(prior_alpha, prior_beta).log()  # normalization constants
        kld += - (beta - 1) / beta
        kld += torch.stack([1. / (i + alpha * beta) * self.beta_func(i / alpha, beta) for i in range(1, 11)]).sum(axis=0) * (prior_beta - 1) * beta  # 10th-order Taylor approximation
        # TODO: use the mean or the sum?
        kl_qz_pz = torch.mean(torch.sum(kld, dim=1), dim=0)
        # kld_loss = toch.mean(-0.5 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim = 1), dim = 0)
        assert kl_qz_pz >= 0, "Stoped due to invalid KL-divergence loss. "

        clustering_loss = self.alpha_kl_z * kl_qz_pz

        # # Clustering loss for maximum sparsity, meaning only one dimension should be used!
        # # TODO: does sparsity_loss makes sense for stick-breaking vae?
        ...
        # # Component constraint classificfation learning
        ...
        if False and self._n_train_steps_mixture == 0:
            ...
            helper_functions.print_loss_params_relation(
                ...
                [('FC Alpha', self.encoder.fc_alpha.named_parameters()),
                 ('FC Beta', self.encoder.fc_beta.named_parameters()),
        ...
        if TB.LOG_INTERVAL > 0 and TB.TI_LOG_STEP % TB.LOG_INTERVAL == 0:
            ...
            TB.TENSORBOARD_LOGGER.add_scalar('training/ti_classification_acc',
                                             (torch.argmax(latent_variables, dim=-1) == targets).float().mean().item(),
