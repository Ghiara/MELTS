...
class TrainingAlgorithm:
    ...
    def train(self):
        ...
        #                    for el in l]
        showcase_epochs = [el for l in [np.linspace(round((self.num_epochs - 1) / np.log2(self.num_showcase) * i_list[i + 0]),
                                                    round((self.num_epochs - 1) / np.log2(self.num_showcase) * i_list[i + 1]),
                                                    int(2 ** i_list[i + 0] + 1) + (i == 0)).astype(np.int)[1:]
                                        for i in range(len(i_list) - 1)]
                           for el in l]
        ...
        self._n_env_steps_total += self.rollout_coordinator.collect_replay_data(
            np.random.permutation(self.train_tasks),
            num_samples_per_task=self.num_transitions_per_episode
        )
        ...
        for epoch in gt.timed_for(range(self.initial_epoch, self.initial_epoch + self.num_epochs), save_itrs=True):
            ...
            self.reconstruction_trainer.train(self.num_reconstruction_steps, current_epoch=epoch)
            ...
            self.get_gaussian_factors()  # save the embeddings
    ...
    def get_gaussian_factors(self, batch_size=1024, num_samples_per_class=100):
        ...
        data, _ = self.replay_buffer.sample_random_few_step_batch(train_indices, self.num_classes * num_samples_per_class, normalize=self.use_data_normalization)
        ...
        TB.TENSORBOARD_LOGGER.add_embedding(emb,
                                            metadata=[f'{t.item()} [{int(s * 100) / 100 if type(s) is np.float64 else [int(k * 100) / 100 for k in s]}] -> {predicted_classes[i].item()}'
                                                      for i, (t, s) in enumerate(zip(targets, specs))],
                                            global_step=TB.TI_LOG_STEP)
        return d
    ...
    def generate_augmented_data(self):
        ...
        for step_nr in range(self.augmented_rollout_steps):
            ...
            data, _ = self.replay_buffer.sample_random_few_step_batch(indices, self.augmented_rollout_batch_size,
    ...
    def showcase_task_inference(self):
        results = self.rollout_coordinator.collect_data(self.test_tasks, 'train', deterministic=True,
                                                        num_samples_per_task=self.num_transitions_per_episode, animated=False)
