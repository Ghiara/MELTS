...
class StackedReplayBuffer:
    ...
    def get_indices(self, points, batch_size, prio=None):
        ...
        # match_map = np.where(points - self.time_steps > self._first_timestep[points])
        # points = points[match_map]
    ...
    def sample_few_step_batch(self, points, batch_size, normalize=True):
        ...
        #     data = self.normalize_data(data)
        #
        # # Filter samples that mix multiple base_task
        # indices = np.where((data["true_tasks"][:, 0, 0] == data["true_tasks"][:, -1, 0])
        #                    | (data["true_tasks"][:, 0, 0] == 0))[0]
        # # TODO: Change in case keys are changed
        # # Set data outside of trajectory to 0
        # data['observations'][match_map] = 0.
        # data['next_observations'][match_map] = 0.
        ...
        # for key in data:
        #     data[key] = np.reshape(data[key], (batch_size, self.time_steps + 1, -1))
        #     data[key] = np.take(data[key], indices, axis=0)
        # points = np.take(points, indices, axis=0)
        # return data, points

    def sample_episode(self, points, batch_size, normalize=True):
        points = self._first_timestep[points]
        all_indices = points[:, None] + np.arange(0, self.episode_length)[None, :]
        data = self.sample_data(all_indices)
        ...
        for key in data:
            data[key] = np.reshape(data[key], (batch_size, self.episode_length, -1))
        return data
    ...
    def sample_random_few_step_batch(self, points, batch_size, normalize=True, prio=None, return_sac_data=False):
        ...
        data = self.sample_few_step_batch(indices, batch_size, normalize=normalize)
        if not return_sac_data:
            return data, self.sample_episode(indices, batch_size, normalize=normalize)
        else:
            return data, self.sample_data(indices)
        # indices = self.get_indices(points, batch_size, prio=prio)
        # if not return_sac_data:
        ...
        # factor = 4
        # indices = self.get_indices(points, batch_size * factor, prio=prio)
        #
        # data, selected_indices = self.sample_few_step_batch(indices, batch_size * factor, normalize=normalize)
        # for key in data:
        #     data[key] = data[key][:batch_size, :, :]
        # selected_indices = selected_indices[:batch_size]
        # if not return_sac_data:
        #     return data, self.sample_episode(selected_indices, batch_size, normalize=normalize)
        # else:
        #     return data, self.sample_data(selected_indices)
